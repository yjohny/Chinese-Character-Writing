...
- Two floating buttons appear during practice: **"Show me"** (bottom-left, orange) calls `skipAsUnknown()` to enter the incorrect/stroke-review flow when the user doesn't know the character — visible only during `.writing`. **"I got it right"** (bottom-right, green) calls `overrideCorrect()` for when recognition misidentifies valid handwriting — visible only during `.recognizing` and `.incorrect`, and only when the user has drawn enough strokes (`hasDrawnEnoughForOverride`: at least `max(2, expectedStrokes/2)`). This prevents gaming by tapping the override without actually writing. The incorrect screen pauses for 2 seconds before auto-transitioning to stroke order, giving legitimate users time to tap the override.
